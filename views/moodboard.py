...
def add_new_prompt(image_number, prompt_text):
    try:
        ...
        VALUES (:sno, :prompt_text, 10, 'Null')
        """)
        with engine.connect() as conn:
...
def add_comments(sno, comments):
    try:
        ...
        with conn.cursor() as cursor:
            cursor.execute(update_query, (comments, int(sno)))
            conn.commit()

            # Check if rows were updated
            if cursor.rowcount > 0:
                st.success("Comments added successfully!")
                logger.info(f"Comments added to serial_nos {sno}.")
            else:
...
with col1:
    ...
    # Image rating slider
    image_review = st.slider(f"Rate Image ", 1, 10, value=image_review_score, format="%d")
    ...
    <style>
        div[data-testid="stTextArea"] {
            margin-top: -30px;  /* Move the input box upwards */
        }
    </style>
""", unsafe_allow_html=True)
...
#      <style>
#          div[data-testid="stTEXTAREA"] {
#              margin-top: -10px;  /* Move the input box upwards */
